  ...
  async fn voice_state_update(
    ...
    if Some(parent.id,) == left_channel_id || Some(parent.id,) == joined_channel_id {
      ...
      if let Some(cap,) = parent.capacity {
        map
          .insert("user_limit".into(), cap.to_string().into(),)
          .drop();
      }
      let mut new = ctx
        .http
      ...
      voice_channels::updater::update_channel(UpdaterContext {
        ...
        total_children_number,
        users_connected_capacity: new
          .user_limit
          .ok_or_else(|| eyre!("No user limit provided for channel with id {channel_id}!"),)?,
    ...
    } in children
    {
      ...
      voice_channels::updater::update_channel(UpdaterContext {
        ...
        total_children_number,
        users_connected_capacity: channel
          .user_limit
          .ok_or_else(|| eyre!("No user limit provided for channel with id {child_id}!"),)?,
...
impl ParsedVoiceStateEvent {
  fn member(&self,) -> &Member {
    match self {
      | Self::Joined { member, .. } | Self::Left { member, .. } | Self::Changed { member, .. } =>
        member,
    }
  }
}
    ...
    async move {
      ...
      let res = match event.clone() {
        ...
        | Event::GuildDelete(mut guild_delete_event,) => {
          ...
          self
            .on_guild_leave(ctx, guild_id, guild,)
