...
fn main() -> Result<(),> {
  ...
  if let Err(err,) = dotenv() {
    if err.not_found() {
      eprintln!(
        "Not .env file was found! This is not necessarily a fatal error if you have all necessary \
         environment variables configured in your environment."
      );
    } else {
...
async fn on_dispatch_error_hook(
  ...
  let to_send = match error {
    ...
    | DispatchError::Ratelimited(info,) => {
      format!("You've been ratelimited and are unable to send commands!: details: `{info:#?}`")
    },
    ...
    | DispatchError::NotEnoughArguments { min, given, } => format!(
      "Not enough arguments provided for command `{command}`! Minimum arguments: `{min}`, given \
       arguments: `{given}`"
    ),
    | DispatchError::TooManyArguments { max, given, } => format!(
      "Too many arguments provided for command `{command}`! Maximum arguments: `{max}`, given \
       arguments: `{given}`"
    ),
