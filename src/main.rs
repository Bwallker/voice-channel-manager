...
async fn on_command_error(error: FrameworkError<(), Report>) -> Result<()> {
    if let FrameworkError::Setup {error, ..} = error {
        return Err(error);
    }
    let channel_id = match error {
        |
    };
}
...
async fn start() -> Result<()> {
    ...
    let framework = Framework::builder()
        .setup(|ctx, _ready, framework| Box::pin(async move { 
            register_globally(ctx, &framework.options().commands).await.wrap_err_with(|| eyre!("Failed to register commands!"))?;
            Ok(())
        }))
        .options(FrameworkOptions {
            on_error: |error| Box::pin(on_command_error(error)),
