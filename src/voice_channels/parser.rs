...
impl<'a> Parser<'a> {
    ...
    fn advance(&mut self) {
        let Some(c) = self.current_char() else { return };
    ...
    fn parse_string(&mut self) -> Result<TemplatePart> {
        ...
        if start_idx == self.current_idx {
            return Err(eyre!(
                "Unexpectedly parsed empty string, this should never happen! At {}:{} with current char: {}",
                self.current_col,
                self.current_row,
    ...
    fn parse_template_content(&mut self) -> Result<TemplatePart> {
        let ret = Ok(match self.current_byte().ok_or_else(|| eyre!("Unexpected end of input at {}:{}. Reached end of input while trying to parse template content.", self.current_col, self.current_row))? {
