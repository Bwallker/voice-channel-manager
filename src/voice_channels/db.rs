...
pub(crate) async fn set_template(
  ...
  query!(
    "INSERT INTO template_channels (channel_id, guild_id, channel_template, next_child_number) \
     VALUES ($1, $2, $3, 1) ON CONFLICT (channel_id) DO UPDATE SET channel_template = $3;",
    channel_id.0 as i64,
    guild_id.0 as i64,
    template
  )
...
pub(crate) async fn register_child(
  ...
  let row = query!(
    "UPDATE template_channels SET next_child_number = next_child_number + 1 WHERE channel_id = $1 \
     AND guild_id = $2 RETURNING next_child_number - 1 AS child_number;",
    parent_id.0 as i64,
    guild_id.0 as i64
  )
  ...
  query!(
    "INSERT INTO child_channels (guild_id, parent_id, child_id, child_number) VALUES ($1, $2, $3, \
     $4) ON CONFLICT (child_id) DO NOTHING;",
    guild_id.0 as i64,
    parent_id.0 as i64,
    child_id.0 as i64,
    row.child_number
