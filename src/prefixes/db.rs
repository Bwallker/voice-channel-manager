...
pub async fn get_prefix<'a>(executor: &PgPool, guild_id: GuildId) -> Result<Option<String>> {
    query!(
...
pub async fn set_prefix<'a>(executor: &PgPool, guild_id: GuildId, prefix: String) -> Result<()> {
    query!(
...
pub async fn delete_prefix<'a>(executor: &PgPool, guild_id: GuildId) -> Result<()> {
    query!(
