...
pub(crate) async fn clean_inactive_guilds_from_db(
    ...
    .wrap_err_with(|| {
        eyre!(
            "Deleting child channels from database for inactive guilds failed! Active guild ids \
             were {guilds_to_keep:?}!"
        )
    },)
    .map(|res| {
    ...
    .wrap_err_with(|| {
        eyre!(
            "Deleting template channels from database for inactive guilds failed! Active guild \
             ids were {guilds_to_keep:?}!"
        )
    },)
    .map(|res| {
        info!(
