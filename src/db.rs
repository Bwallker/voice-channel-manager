...
pub(crate) async fn clean_left_guild_from_db(executor: &PgPool, guild_id: GuildId,) -> Result<(),> {
  ...
  .execute(&mut *transaction)
  .await
  .wrap_err_with(|| eyre!("Deleting prefixes from database for guild with ID `{guild_id}` failed!"))
  .map(|res| {
    info!(
...
pub(crate) async fn clean_inactive_guilds_from_db(
  ...
  .wrap_err_with(|| {
    eyre!(
      "Deleting child channels from database for inactive guilds failed! Active guild ids were \
       {guilds_to_keep:?}!"
    )
  },)
  .map(|res| {
  ...
  .wrap_err_with(|| {
    eyre!(
      "Deleting template channels from database for inactive guilds failed! Active guild ids were \
       {guilds_to_keep:?}!"
    )
  },)
  .map(|res| {
  ...
  query!(
    "DELETE FROM prefixes WHERE NOT guild_id = ANY($1);",
    guilds_to_keep
  )
  .execute(&mut *transaction,)
  .await
  .wrap_err_with(|| {
    eyre!(
      "Deleting prefixes from database for inactive guilds failed! Active guild ids were \
       {guilds_to_keep:?}!"
    )
  },)
  .map(|res| {
